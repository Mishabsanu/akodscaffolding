  ...
  providers: [
    CredentialsProvider({
      ...
      credentials: {
        ...
        kitchen: {
          label: "Kitchen Username",
          type: "text",
          placeholder: "Enter your kitchen username",
        },
        password: {
          label: "Password",
          type: "password",
      ...
      async authorize(cred) {
        ...
        await connectDB();
        const username = isEmailValid(cred?.username)
          ? { email: cred?.username }
          : { username: cred?.username };
        const account = await Accounts.findOne(username).populate({
          path: "kitchens",
          model: Kitchens,
          match: { username: cred?.kitchen },
        });

        if (!account) throw new Error("Account not found.");

        if (cred?.kitchen) {
          if (!verifyPassword(cred?.password, account?.kitchens?.[0]?.password))
          ...
          if (!verifyPassword(cred?.password, account?.password))
    ...
    CredentialsProvider({
      ...
      async authorize(cred) {
        ...
        if (
          !account?.tables?.some?.(
            ({ username }: { username: string }) => username === cred?.table,
          )
        )
