...
const OrderPage = () => {
  ...
  const [filteredProducts, setFilteredProducts] =
    useState<Array<TMenuCustom>>(menus);
  ...
  const showOrderButton = restaurant?.tables?.some(
  ...
  const onMenuScroll = (event: UIEvent<HTMLDivElement>) => {
    ...
    if (scrollTop > 30) {
      ...
      if (order?.current && scrollTop >= order?.current?.offsetTop - 15)
  ...
  const onCategoryScroll = (event: SyntheticEvent) => {
    ...
    if (
      Math.round(target.scrollWidth - target.scrollLeft) - 50 >
      target.clientWidth
    )
  ...
  const onCategoryClick = (categoryName: string) => {
    ...
    if (category.includes(categoryName))
      newCategory = category.filter((item) => item !== categoryName);
      ...
      if (product._id === item._id) {
        ...
        if (item.quantity === 0) {
          const filter = selection.filter(
            (tempItem) => tempItem._id !== product._id,
          );
          selection = [...filter];
        }
      }
      ...
      menus?.filter?.(
        ({ name, description, category: cat }) =>
          (search
            ...
            : true) && (category.length ? category.includes(cat) : true),
      ),
    );
  ...
  useEffect(() => {
    params.set({
      category: category
        .filter((e) => restaurant?.profile.categories.includes(e))
        .join(","),
    });
  ...
  useEffect(() => {
    setHasImageItems(
      filteredProducts?.some((product) => !!product.image) ?? false,
    );
    setHasNonImageItems(
      filteredProducts?.some((product) => !product.image) ?? false,
    );
  ...
  useEffect(() => {
    ...
    restaurant?.username,
    session.data?.restaurant?.username,
    session.status,
  ]);
  ...
  return (
    <div className="orderPage">
      <div className="mainContainer" onScroll={onMenuScroll}>
        <div
          className={`mainHeader ${searchActive ? "searchActive" : ""} ${floatHeader ? "floatHeader" : ""}`}
        >
          <h1>
          ...
          <div className="options">
            ...
            {(!session.data?.role || !showOrderButton) && (
              <Button
                ...
                label={showOrderButton ? "Order" : "Scan"}
        ...
        {!restaurant ? (
          ...
          <div className="order" ref={order}>
            ...
            {hasImageItems && (
              <div
                className={`itemContainer ${!eligibleToOrder ? "restrictOrder " : ""}`}
              >
                <div>
                    ...
                    (item, key) =>
                      !item.hidden && (
                        <MenuCard
                          ...
                          showInfo={
                            item._id.toString() === showInfoCard.toString()
                          }
            ...
            {hasNonImageItems && (
              <div
                className={`itemContainer withoutImage ${!eligibleToOrder ? "restrictOrder " : ""}`}
              >
                <div>
                    ...
                    <MenuCard
                      ...
                      quantity={
                        (selectedProducts.some((obj) => obj._id === item._id) &&
                          selectedProducts?.find((obj) => obj._id === item._id)
                            ?.quantity) ||
                        0
                      }
                    />
