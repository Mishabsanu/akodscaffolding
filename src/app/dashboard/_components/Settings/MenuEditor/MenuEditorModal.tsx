...
const MenuEditorModal = (props) => {
  ...
  const menuItemSubmitOnClick = () => {
      ...
      uploadMenuItemImage(
        ...
        (err, imageURL) => {
          ...
          if (props.state === "menuItemEditState") {
            ...
            itemName !== editItem.name && (menuItem.name = titleCase(itemName));
    ...
    menuItemContainer.current.scrollTop =
      menuItemContainer.current.scrollHeight;
  };
  const onCategorySelect = (categories, change) => {
    ...
    if (change.action === "remove-value") {
      return setSelectedCategoryList((list) =>
        list.filter((val) => change.removedValue.value !== val),
      );
    }
  ...
  return (
    <div
      className={`menuEditorModal ${props.state ? `show ${props.state}` : ""}`}
      ref={menuEditorModalRef}
    >
        ...
        <div className="modalHeader">
          <div>
            <span
              onClick={onPrev}
              style={getIconMask(
                `/icons/Base/${stateList.length > 1 ? "arrowLeft" : "plus"}.svg`,
              )}
            />
            <h5 className="heading">{heading}</h5>
          </div>
          <h5 className="uploadProgress">
            {submitLoading ? `${uploadPercent}%` : ""}
          </h5>
        </div>
        <div className="modalContent">
          <div className="newSelector">
            <div
              className="newMenuCategory"
              onClick={() => onNext("menuCategoryState")}
            >
              Menu Category
            </div>
            <div
              className="newMenuItem"
              onClick={() => onNext("menuItemState")}
            >
              Menu Item
            </div>
          </div>
          <div className="menuCategoryContainer">
            ...
            <div className="categoryControls">
              <div className="categoryNameTitle">
                <h6>Category Name</h6>
                <span>
                  {categoryName?.length}/{maxLengthCategory}
                </span>
              </div>
              <TextInput
                ...
                useRef={categoryInput}
              />
              <div className="categoryButtons">
                <Button
                  label={menuItemSubmitLabel}
                  filled
                  round
                  onClick={categorySubmitOnClick}
          ...
          <div
            ...
            onScroll={onMenuItemContainerScroll}
          >
            <div className="menuItemHead">
              ...
              >
                {!displayImage ? (
                  <span
                    ...
                    style={getIconMask("/icons/Base/cutlery.svg")}
                  />
                ) : (
                  <span
                    className="image"
              ...
              <div className="itemNameContainer">
                <div className="itemNameTitle">
                  <h5>Item Name</h5>
                  <span>
                    {itemName?.length}/{maxLengthItemName}
                  </span>
                </div>
                <TextInput
                  placeholder="Enter item name"
                  maxLength={maxLengthItemName}
                  value={itemName}
                  setValue={setItemName}
                  useRef={itemNameInput}
                />
              </div>
            </div>
            <div className="menuItemBody">
              <ToggleButton
                className="vegSelector"
                items={["veg", "non veg", "contains egg"]}
                active={itemVeg}
                setActive={setItemVeg}
              />
              <div className="itemPrice">
                <div className="itemPriceLabel">
                  <h6>Item Price</h6>
                  <span>
                    {itemPrice?.length}/{maxLengthItemPrice}
                  </span>
                </div>
                <TextInput
                  placeholder="Enter item price"
                  maxLength={maxLengthItemPrice}
              ...
              <div className="itemDescription">
                <div className="itemDescriptionLabel">
                  ...
                  <span>
                    {itemDesc?.length}/{maxLengthItemDesc}
                  </span>
                </div>
                <textarea
                  ...
                  value={itemDesc}
                  onChange={(e) =>
                    setItemDesc(e.target.value.slice(0, maxLengthItemDesc))
                  }
                />
              </div>
              <div className="itemCategory">
                ...
                <Select
                  ...
                  styles={selectStyles}
                  menuPosition="fixed"
                  menuPortalTarget={menuEditorModalRef.current}
                  isSearchable
                  isClearable
                  isMulti
                  ...
                  value={selectedCategoryList.map((item) => ({
                    value: item,
                    label: item,
                  }))}
          ...
          <Button
            className="submitMenuItem"
            onClick={menuItemSubmitOnClick}
            filled
            round
            label={menuItemSubmit ? menuItemSubmitLabel : "More"}
            loading={submitLoading}
          />
        </div>
      </div>
