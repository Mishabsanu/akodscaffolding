...
const OrderRequests = (props: TOrderRequestsProps) => {
  ...
  const [rejectCard, setRejectCard] = useState<{
    _id: string | null;
    details: boolean;
  }>({ _id: null, details: false });
  ...
  useEffect(() => {
    if (orderRequest.length === 0) {
      ...
      !orderRequest.some(({ _id }) => _id.toString() === activeCardID)
    ) {
    ...
    <div className="orders">
      {orderRequest?.length === 0 ? (
        ...
        <div className="ordersContent">
          <div
            className={`list ${orderActionLoading ? "disable" : ""}`}
            onScroll={onScroll}
          >
          ...
          <div
            className={`details ${activeCardData && rejectCard._id === activeCardData._id.toString() ? "reject " : ""}`}
          >
            {!activeCardData ? (
              <NoContent
              ...
              <OrderDetail
                ...
                reject={
                  activeCardData &&
                  rejectCard._id === activeCardData._id.toString()
                }
              />
            )}
          </div>
      ...
      <SideSheet
        title={[activeCardData ? `Table: ${activeCardData?.table}` : ""]}
        open={sideSheetOpen}
